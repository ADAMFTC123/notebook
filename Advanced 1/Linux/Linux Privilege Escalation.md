...
if we want it to run we need to give it excecute perms 
![[Pasted image 20251109231510.png]]
...
**you want  a script or a binary you can maniupulate, if you see in the strings of a root program/binary**

**that it calls for a binary/command for example we have:**
**![[Pasted image 20251110134938.png]]**
**and we know it calls "thm" command. we can find a directory that is writable and make a  file called "thm" and make the thm program open a shell.** 
**we will put that directory first in the $PATH, so it looks in it first when searching for binaries.**
**and than out thm will run with the program that called it privs.**
...
![[Pasted image 20251109232908.png]]

if the suid bit is set than before running system will reset important enc variables like path for example, so the exploit will fail

![[Pasted image 20251109233041.png]]
...
##### how to find writable folders;
find / -writable 2>/dev/null
look for folders you have write access to under home

![[Pasted image 20251109233151.png]]

We see a number of folders under /usr, thus it could be easier to run our writable folder search once more to cover subfolders.
![[Pasted image 20251109233226.png]]
...
in this example lets write to /tmp


lets add it to the PATH so it will look for binaries there
(do it after you set the binary in /tmp with the right privs and all)
...
 for example finding a root SSH private key on the target system and connecting via SSH with root privileges.
...
![[Pasted image 20251110135508.png]]The critical element for this privilege escalation vector is the “no_root_squash” option you can see above. By default, NFS will change the root user to nfsnobody and strip any file from operating with root privileges. If the “no_root_squash” option is present on a writable share, we can create an executable with SUID bit set and run it on the target system.

We will start by enumerating mountable shares from our attacking machine.
![[Pasted image 20251110135618.png]]

We will mount one of the “no_root_squash” shares to our attacking machine and start building our executable.
![[Pasted image 20251110135654.png]]

As we can set SUID bits, a simple executable that will run /bin/bash on the target system will do the job.

![[Pasted image 20251110135704.png]]

Once we compile the code we will set the SUID bit.
![[Pasted image 20251110135722.png]]

You will see below that both files (nfs.c and nfs are present on the target system. We have worked on the mounted share so there was no need to transfer them).
![[Pasted image 20251110135756.png]]